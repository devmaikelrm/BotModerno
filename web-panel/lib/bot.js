...
const STEPS = ['awaiting_name','awaiting_model','awaiting_works','awaiting_bands','awaiting_obs','confirm'];
...
export function getBot(){
  ...
  bot.on('text', async (ctx)=>{
    ...
    if (!draft) return; // ignore casual chat
    ...
    switch(draft.step){
      ...
      case 'confirm':
        if (/^s[ií]$/i.test(t)){
          ...
          await db.from('submission_drafts').delete().eq('tg_id', String(ctx.from.id));
          return ctx.reply('✅ Enviado para revisión. ¡Gracias!');
        } else {
          await db.from('submission_drafts').delete().eq('tg_id', String(ctx.from.id));
          return ctx.reply('❌ Cancelado.');
        }
